  ...
  return (
    <div className="flex flex-wrap justify-center lg:gap-6 gap-9 mt-10">
      {items.map((item, index) => (
        <div
          key={index}
          className={`lg:w-72 w-[${wid}vh] bg-white shadow-lg overflow-hidden flex flex-col relative cursor-pointer`}
        >
          {/* Image */}
          <img src={item.image} alt={item.title} className="w-full lg:h-40 h-48 object-cover" />
          ...
          <div className="lg:p-4 p-6 flex flex-col flex-grow">
            <h2 className={`${item.b=="n"?"mt-10":""} lg:text-2xl text-xl font-semibold`}>{item.title}</h2>
